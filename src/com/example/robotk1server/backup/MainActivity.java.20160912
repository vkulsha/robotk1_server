...
public class MainActivity extends Activity implements OnTouchListener {
	...
	final int RECIEVE_MESSAGE_BT_CONNECT = 1, RECIEVE_MESSAGE_BT_CONNECTED = 2,
			...
			RECEIVE_MESSAGE_TTS_STOP = 5;// Статус для Handler
	private BluetoothAdapter btAdapter = null;
	private StringBuilder sb = new StringBuilder();
	private static BTConnectedThread mBTConnectedThread;// bluetooth server
	private static final UUID MY_UUID = UUID
			.fromString("00001101-0000-1000-8000-00805F9B34FB");// SPP UUID
																// сервиса
		...
		if (btAdapter == null) {
			...
			if (btAdapter.isEnabled()) {
				...
				Toast.makeText(this.getApplicationContext(),
						"Bluetooth выключен, работаем через ADB!",
						Toast.LENGTH_SHORT).show();
				// errorExit("Fatal Error",
				// "Включите Bluetooth и перезапустите программу!");
		...
		public void run() {
			try {
				while (true) {
					...
					ServerAsyncTask serverAsyncTask = new ServerAsyncTask();// асинхр
																			// задача
																			// для
																			// нового
																			// планшета
			...
			while (true) {
				try {
					...
					h.obtainMessage(RECIEVE_MESSAGE_BT_CONNECTED, bytes, -1,
							buffer).sendToTarget(); // Отправляем в очередь
													// сообщений Handler
				} catch (IOException e) {
					break;
				}
